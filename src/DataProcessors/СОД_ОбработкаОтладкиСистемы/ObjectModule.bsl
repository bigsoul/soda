...
Процедура РегистрацииТранспортПоток_Чтение(ДанныеПолучателя) Экспорт
	...
	Попытка
		...
		Если ДанныеПолучателя.Узел.ПротоколДоставкиОтправлений = Перечисления.СОД_ПротоколыДоставкиОтправлений.WEBСервис Тогда
		...
		Для Каждого Регистрация Из ДанныеПолучателя.Строки Цикл
			...
			ИначеЕсли ДанныеПолучателя.Узел.ПротоколДоставкиОтправлений = Перечисления.СОД_ПротоколыДоставкиОтправлений.HTTPСервис Тогда
				СтрокаВозврата = СОД_Синхронизация.ВыполнитьHSЗапросRun(ДанныеПолучателя.Узел, СтрокаРегистрации); 
			ИначеЕсли ДанныеПолучателя.Узел.ПротоколДоставкиОтправлений = Перечисления.СОД_ПротоколыДоставкиОтправлений.AMQP Тогда
						
				Клиент = СОД_Синхронизация.ПолучитьAMQPКлиентHTTP(ДанныеПолучателя.Узел);  
				
				СОД_Синхронизация.ОбновитьМаршрутAMQPHTTP(Клиент, ДанныеПолучателя.Узел, Перечисления.СОД_НаправлениеРегистрации.Исходящее);
				
				СтрокаВозврата = СОД_Синхронизация.ТранспортAMQPИсходящееHTTP(Клиент, ДанныеПолучателя.Узел, СтрокаРегистрации);
			...
			Если СтрокаВозврата = "Успех" Тогда
				...
				Если РегистрацияОбъект.ИсходящееПодтверждение
					И РегистрацияОбъект.ВходящееПодтверждение Тогда
	...
	Попытка
		...
		Для Каждого Регистрация Из ДанныеПолучателя.Строки Цикл
			...
			ИначеЕсли ДанныеПолучателя.Узел.ПротоколДоставкиОтправлений = Перечисления.СОД_ПротоколыДоставкиОтправлений.AMQP Тогда
				...
				СОД_Синхронизация.ОбновитьМаршрутAMQPПодтверждениеHTTP(Клиент, ДанныеПолучателя.Узел, Перечисления.СОД_НаправлениеРегистрации.Исходящее);
				
				СтрокаВозврата = СОД_Синхронизация.ТранспортПодтверждениеAMQPИсходящееHTTP(Клиент, ДанныеПолучателя.Узел, СтрокаРегистрации);
...
Процедура РегистрацииТочкаАктуальности() Экспорт 
	...
	Попытка  
		...
		Для Каждого Узел Из СписокУзлов Цикл
			...
			Если ЗначениеЗаполнено(ДатаОтправленныхМин)
				И ЗначениеЗаполнено(ДатаОтправленныхМакс)
				И ЗначениеЗаполнено(ВОжиданииМин)
				И ЗначениеЗаполнено(ВОжиданииМакс) Тогда
			...
			ИначеЕсли ЗначениеЗаполнено(ДатаОтправленныхМин)
				И ЗначениеЗаполнено(ДатаОтправленныхМакс)
				И Не ЗначениеЗаполнено(ВОжиданииМин)
				И Не ЗначениеЗаполнено(ВОжиданииМакс) Тогда
				
				// в базе все регистрации отправленные, поэтому точка актуальности текущая дата (-1 секунда)
				
				ТочкаАктуальности = УниверсальноеВремя( ТекущаяДатаСеанса() - 1 );
				
			ИначеЕсли Не ЗначениеЗаполнено(ДатаОтправленныхМин)
				И Не ЗначениеЗаполнено(ДатаОтправленныхМакс)
				И ЗначениеЗаполнено(ВОжиданииМин)
				И ЗначениеЗаполнено(ВОжиданииМакс) Тогда
	...
	Попытка 
		...
		Попытка 
			...
			ИначеЕсли Узел.ПротоколДоставкиОтправлений = Перечисления.СОД_ПротоколыДоставкиОтправлений.AMQP Тогда
				...
				СОД_Синхронизация.ОбновитьМаршрутAMQPТочкаАктуальностиHTTP(Клиент, Узел, Перечисления.СОД_НаправлениеРегистрации.Исходящее);
	...
	Для Каждого ДанныеПолучателя Из ДанныеПоПолучателям Цикл
		...
		Попытка
			...
			Для Каждого СтрокаРегистрации Из ДанныеРегистраций Цикл
				...
				РегистрацииКОбработке = ДанныеРегистраций.НайтиСтроки(Новый Структура("ГУИД,Обработано", СтрокаРегистрации.ГУИД,Ложь));
				
				КоличествоКОбработке = РегистрацииКОбработке.Количество();
				
				Если КоличествоКОбработке Тогда
					
					Если РегистрацииКОбработке[КоличествоКОбработке - 1].Направление = Перечисления.СОД_НаправлениеРегистрации.Исходящее Тогда
					...
					Иначе
						...
						Для Номер = 0 По КоличествоКОбработке - 1 Цикл
							...
							Иначе	
								...
								Обработано = Не (ТекущаяРегистрация.Направление = Перечисления.СОД_НаправлениеРегистрации.Входящее);
...
Процедура РегистрацииУдаление(ГраницаУдаления, Количество) Экспорт
	...
	ДатаГраницыУдаления				= УниверсальноеВремя( НачалоДня(ТекущаяДатаСеанса()) - ГраницаУдаления * 24 * 60 * 60 );
...
Функция Справочник_СОД_Регистрации_Запись(ЭтаСтруктура) Экспорт
	...
	ИначеЕсли ЭтаСтруктура.Ссылка.Метод = "ТранспортПодтверждение" Тогда
		...
		Если ЭтиДанные.ИсходящееПодтверждение
			И ЭтиДанные.ВходящееПодтверждение Тогда
...
Функция ПланОбмена_СОД_Узлы_Чтение(Ссылка) Экспорт
	...
	ЭтаСтруктура.Вставить("Ссылка", ПолучитьПолнуюСсылку(Ссылка));
	ЭтаСтруктура.Вставить("Код", 	Ссылка.Код);
...
Функция РежимЗаписи(ЭтиДанные, ЭтаСтруктура) Экспорт
	...
	Иначе
		...
		Если ЭтаСтруктура.ПометкаУдаления
			И ЭтиДанные.Ссылка.Проведен Тогда
			
			РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения;
			
		ИначеЕсли Не ЭтаСтруктура.Проведен
			И ЭтиДанные.Ссылка.Проведен Тогда
...
Функция ЧтениеСоставногоТипа(ЭтиДанные) Экспорт
	...
	Иначе
		...
		УстановитьБезопасныйРежим(Истина);
			ЭтиДанные = Вычислить("МодульПравил." + ИмяМетаданныхВИсточнике + "_Чтение(ЭтиДанные, Истина)");
		УстановитьБезопасныйРежим(Ложь);
...
Функция ПолучитьПолнуюСсылку(Значение) Экспорт
	...
	ИначеЕсли ПолнаяСсылка.Группа = "РегистрСведений" Тогда
		...
		Для Каждого Отбор Из Значение.Отбор Цикл
			...
			ОтборСтруктура.Вставить(Отбор.Имя, Новый Структура("Использование, ВидСравнения, Значение", Отбор.Использование, Отбор.ВидСравнения, ЗначениеОтбора));
...
Функция ОбработатьРегистрацию(Регистрация, Обработано = Ложь) Экспорт
	...
	Попытка
		...
		Если Не Обработано Тогда
			...
			СтруктураОбъекта = СОД_Синхронизация.ЗначениеИзСтроки(РегистрацияОбъект.ТекстовыеДанные, Регистрация.ФорматСообщения).ТекстовыеДанные;
						
			ИмяМетаданныхВИсточнике = СтруктураОбъекта.Ссылка.Группа + "_" + СтруктураОбъекта.Ссылка.Имя;
			ИмяМетаданныхВПриемнике = СоответствиеМетаданных[ИмяМетаданныхВИсточнике];

			ИмяМетодаПравила = ИмяМетаданныхВПриемнике + "_Запись(СтруктураОбъекта)";
