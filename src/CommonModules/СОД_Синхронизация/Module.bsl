...
Функция ПолучитьМодульСистемы() Экспорт
	...
	ИсточникМодуля = СОД_МаршрутизацияВызовСервераПовтИсп.ОбработкаОтладкиСистемы();
...
// Параметры:
//  Узел					- ПланОбменаСсылка.СОД_Узлы 	- Узел, в который необходимо отправить эту регистрацию
...
Функция Зарегистрировать(Узел, Ссылка, ОтправлениеСтруктура, УдалениеОбъекта = Ложь) Экспорт
	...
	Попытка
		...
		ФорматСообщения = СОД_СинхронизацияВызовСервераПовтИсп.ПолучитьФорматСообщенияУзла(Узел);
...
Процедура ОбновитьМаршрутAMQPHTTP(Клиент, Узел, Направление) Экспорт
	...
	Если Направление = Перечисления.СОД_НаправлениеРегистрации.Исходящее Тогда
		...
		Клиент.ОтправитьДляОбработки(HTTPЗапрос); 
...
Процедура ДатыВСтроки(Значение) Экспорт
...
Процедура ВидыСравненияВСтроку(Значение) Экспорт
...
Процедура ТаблицыЗначенийВМассив(Значение) Экспорт
...
Процедура ДобавитьПротоколОбменаРегистрациями(Входящее, Данные) Экспорт
	...
	Если Не СОД_СинхронизацияВызовСервераПовтИсп.ВключитьПротоколОбменаРегистрациями() Тогда
		Возврат;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Данные) Тогда
		РегистрацияСтруктура = ЗначениеИзСтроки(Данные);
	Иначе
...
Процедура РегистрацииТранспорт(СтруктураПараметров = Неопределено) Экспорт
	...
	СОД_Обмен = ПолучитьМодульСистемы();
