...
Функция СоздатьНовыйФайл() Экспорт
	...
	Если Не СОД_СинхронизацияПовтИсп.СоздаватьФайлLog() Тогда
		Возврат "";
	КонецЕсли;
	
	ПолноеИмяФайла = СОД_СинхронизацияПовтИсп.ПутьККаталогуФайлаLog() + "\" + СтрЗаменить(УниверсальноеВремя( ТекущаяДатаСеанса() ), ":", "-") + ".txt";
